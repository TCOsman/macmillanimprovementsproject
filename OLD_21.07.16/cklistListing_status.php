...
$get = $connect->query("SELECT 
						  ...
						  GROUP BY r.ckListID, r.volID, v.volName, v.volSurname, r.ckListStatus 
						  ORDER BY r.ckListStatus, v.volName
						  LIMIT $start, $per_page");		  
			...
			<div id="title">Recruitment Checklist Listing</div>
				...
				echo "<br />"; 
						...
						echo "<div id='pagination'>"; // ====== pagination 			
							if (!($start<=0))
								echo "<a href='cklistListing_status.php?start=$prev'>&#9668</a>";
							...
							for ($x = 0; $x < $record_count; $x = $x + $per_page)
									{
										if ($start != $x)
											echo "<a href='cklistListing_status.php?start=$x'>$i</a>";
										else
											echo "<a href='cklistListing_status.php?start=$x'><b>$i</b></a>";
										$i++;
									}

							//show next button
							if (!($start >= $record_count - $per_page))
								   echo "<a href='cklistListing_status.php?start=$next'>&#9658</a>";
						echo "</div>"; 								   
						?> 
