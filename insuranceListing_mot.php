...
$query = "SELECT i.insID, i.insPolicy, i.insExpDate, i.MOT, i.insNote, i.vehID, i.volID, 
							...
							WHERE i.MOT >= '".$startDate."'
							AND   i.MOT <= '".$endDate."'
							AND i.volID = v.volID 
                            AND i.vehID = h.vehID
                            GROUP BY i.insID, i.insPolicy, i.insExpDate, i.MOT 
							ORDER BY i.MOT";
...
// to set a variable which holds the query results - Ordered by MOT expiration date (only active ones)
			...
			<br />
				...
				echo "<br />";										
						...
						echo "<div id='pagination'>"; // ====== pagination 			
							if (!($start<=0))
								echo "<a href='insuranceListing_mot.php?start=$prev'>&#9668</a>";
							...
							for ($x = 0; $x < $record_count; $x = $x + $per_page)
									{
										if ($start != $x)
											echo "<a href='insuranceListing_mot.php?start=$x'>$i</a>";
										else
											echo "<a href='insuranceListing_mot.php?start=$x'><b>$i</b></a>";
										$i++;
									}

							//show next button
							if (!($start >= $record_count - $per_page))
								   echo "<a href='insuranceListing_mot.php?start=$next'>&#9658</a>";
						echo "</div>"; 								   
						?> 
